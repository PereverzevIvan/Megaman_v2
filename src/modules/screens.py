...
class StartScreen(InteractionScreen):
    def __init__(self, screen: Surface):
        ...
        self.variants = [
            Button(int(self.w // 2), int(self.h // 1.6), self.text_font_size, 'GAME START', self.screen),
            Button(int(self.w // 2), int(self.h // 1.4), self.text_font_size, 'SETTINGS', self.screen),
...
class SettingsScreen(InteractionScreen):
    def __init__(self, screen: Surface):
        ...
        self.btn_save = Button(int(self.w // 2.5), int(self.h // 1.1),
                               self.text_font_size, '    SAVE    ', self.screen)
        self.btn_def = Button(int(self.w // 1.64), int(self.h // 1.1), self.text_font_size, 'CANCEL', self.screen)

        self.slider_volume = Slider(WIDTH // 2, HEIGHT // 2, round(WIDTH / 3.2), HEIGHT // 12, self.screen)
        self.resolution_spinbox = SpinBox(WIDTH // 2, HEIGHT // 3, 50, ['1280x720', '640x360', '1600x900'], self.screen)
        ...
        while run:
            ...
            for event in pg_event.get():
                ...
                if event.type == MOUSEMOTION:
                    ...
                    self.resolution_spinbox.check_hover(mouse_pos)
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    self.resolution_spinbox.update(mouse_pos)
